...
const MistakeAnalysis: NextPage = () => {
  ...
  const { strategy, subject, output, topic, difficulty, time_taken, reason_for_error } =
    router.query as any;
  ...
  const generateFilters = () => {
    if (watch("strategy") != null && watch("subject") != null && watch("output") != null) {
      router.replace(
        `/user/mistake-analytics?strategy=${watch("strategy")}&subject=${watch(
          "subject"
        )}&output=${watch("output")}`,
        undefined,
        {
  ...
  const clearFilters = () => {
    if (watch("strategy") && watch("subject") && watch("output")) {
      router.replace(`/user/mistake-analytics`, undefined, {
        shallow: true,
      });
      reset({ ...defaultValues });
      handleAnalyticsRender("filter", false);
  ...
  const generateSubFilters = () => {
    if (
      ...
      router.replace(
        ...
        }${watch("time_taken") ? `&time_taken=${watch("time_taken")}` : ``}${watch("reason_for_error") ? `&reason_for_error=${watch("reason_for_error")}` : ``
        }`,
        undefined,
        {
          shallow: true,
  ...
  React.useEffect(() => {
    if (strategy || subject || output || topic || difficulty || time_taken || reason_for_error) {
      reset({
        ...defaultValues,
        strategy: strategy ? strategy : null,
        subject: subject ? subject : null,
  ...
  }, [strategy, subject, output, topic, difficulty, time_taken, reason_for_error, reset]);
