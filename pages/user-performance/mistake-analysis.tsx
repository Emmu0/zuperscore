...
// api routes
import { SETTINGS_ENDPOINT, USER_ENDPOINT } from "@constants/api-routes";
...
const Assessments: NextPage = () => {
  ...
  const searchQueryGenerator = (_query: any) => {
    ...
    if (_query && _query?.url) {
      let queryStringArray: any = {};
      _query?.data?.user && (queryStringArray[`user`] = parseInt(_query?.data?.user));
...
export default authWrapper(Assessments, {
  authRequired: true,
